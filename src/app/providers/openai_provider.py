...
class OpenAIProvider(BaseLLMProvider):
    ...
    def __init__(self, model_name: str, api_key: SecretStr | None = None, temperature: float = 0.0) -> None:
        ...
        if self._api_key is not None:
            self._client = OpenAI(api_key=self._api_key.get_secret_value())
    ...
    def invoke(self, prompt: str, system_message: str | None = None) -> str:
        ...
        try:
            ...
            logger.warning(f"OpenAI transient error (will retry): {e}")
            raise RetriableProviderError(f"OpenAI transient error: {e}") from e
    ...
    def stream(self, prompt: str, system_message: str | None = None) -> Iterator[str]:
        """Stream OpenAI chat completion tokens."""
        if not self.is_configured() or self._client is None:
            raise ProviderNotConfiguredError("OpenAI provider not configured.")
        messages: list[ChatCompletionSystemMessageParam | ChatCompletionUserMessageParam] = []
        if system_message:
        ...
        try:
            ...
            logger.warning(f"OpenAI stream transient error (will retry): {e}")
            raise RetriableProviderError(f"OpenAI stream transient error: {e}") from e
        except Exception as e:
            logger.error(f"OpenAI stream failed: {e}")
            raise ProviderError(f"OpenAI stream error: {e}") from e
